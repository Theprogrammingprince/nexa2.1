...
serve(async (req) => {
    ...
    try {
        ...
        const prompt = `You are an expert educational AI tutor specializing in ${course?.department || 'various subjects'}. Your role is to help students understand why they got a question wrong and guide them to the correct understanding. Be encouraging, clear, and pedagogical.
...
Keep your explanation concise but thorough (200-300 words). Use a friendly, encouraging tone.`;
        ...
        // Call Google Gemini API (v1beta with correct model)
        ...
        const aiExplanation = geminiData.candidates?.[0]?.content?.parts?.[0]?.text || 'Unable to generate explanation';
